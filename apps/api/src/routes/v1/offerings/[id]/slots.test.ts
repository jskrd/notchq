...
import route from "./slots.ts";
import { createOffering, createSlot } from "@repo/db/factories/index";
import { Hono } from "hono";
import { describe, expect, it } from "vitest";

const app = new Hono();
app.route("/v1/offerings/:id/slots", route);

describe("GET /v1/offerings/:id/slots", () => {
  it.each(["POST", "PUT", "PATCH", "DELETE"])(
    "%s returns 405",
    async (method) => {
      const response = await app.request("/v1/offerings/1/slots", { method });
  ...
  it.each(["abc", "0", "-1", "1.5"])(
    ...
    async (id) => {
      const response = await app.request(
        `/v1/offerings/${id}/slots?date=2026-03-01`,
      );
  ...
  it("returns 422 when date query param is missing", async () => {
    ...
    const response = await app.request(`/v1/offerings/${offering.id}/slots`);
  ...
  it("returns 422 for invalid date format", async () => {
    const offering = await createOffering();
    const response = await app.request(
      `/v1/offerings/${offering.id}/slots?date=not-a-date`,
    );
    expect(response.status).toBe(422);
  });
  ...
  it.each(["limit=0", "offset=-1"])(
    ...
    async (param) => {
      const offering = await createOffering();
      const response = await app.request(
        `/v1/offerings/${offering.id}/slots?date=2026-03-01&${param}`,
      );
      expect(response.status).toBe(422);
    },
  );
  ...
  it("returns empty array when no slots match", async () => {
    ...
    const response = await app.request(
      `/v1/offerings/${offering.id}/slots?date=2026-03-01`,
    );
